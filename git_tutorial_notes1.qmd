...
Here the program is creating branches for you so that changes are not lost.
The solution to #1 is to pull before you start making ay changes  to make sure you are working on the most up to date version.
The solution to #2 when you both have made changes that need to be reconciled is to

a) Committ your local changes
then you receive the following message:

![]figure/36.png


then you pick from the options
merge 
rebase 
and fast-forward 
